...
public class PhoneOutputAdapterMongo implements PhoneOutputPort {
	...
	public List<Phone> findByPersonId(Integer personId) {
		...
		return telefonoRepositoryMongo.findByPrimaryDuenio_Id(personId).stream()
				.map(telefonoMapperMongo::fromAdapterToDomain)
				.collect(Collectors.toList());
	}

	@Override
	public List<Phone> findByCompany(String company) {
		log.debug("Into findByCompany on Adapter MongoDB");
		return telefonoRepositoryMongo.findByOper(company).stream()
				.map(telefonoMapperMongo::fromAdapterToDomain)
				.collect(Collectors.toList());
	}
}