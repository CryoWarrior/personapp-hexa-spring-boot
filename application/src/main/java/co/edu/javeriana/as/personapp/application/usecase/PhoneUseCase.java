...
public class PhoneUseCase implements PhoneInputPort {
	...
	@Override
	public List<Phone> getPhonesByOwner(Integer personId) throws NoExistException {
		List<Phone> phones = phonePersistence.findByPersonId(personId);
		if (phones.isEmpty()) {
			throw new NoExistException(
					"The person with id " + personId + " does not have phones or does not exist");
		}
		return phones;
	}

	@Override
	public List<Phone> getPhonesByCompany(String company) {
		return phonePersistence.findByCompany(company);
	}
}