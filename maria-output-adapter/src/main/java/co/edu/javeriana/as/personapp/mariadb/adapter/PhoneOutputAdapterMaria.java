...
public class PhoneOutputAdapterMaria implements PhoneOutputPort {
	...
	public List<Phone> findByPersonId(Integer personId) {
		...
		return telefonoRepositoryMaria.findByDuenio_Cc(personId).stream()
	...
	public List<Phone> findByCompany(String company) {
		log.debug("Into findByCompany on Adapter MariaDB");
		return telefonoRepositoryMaria.findByOper(company).stream()
				.map(telefonoMapperMaria::fromAdapterToDomain)
				.collect(Collectors.toList());
	}
}